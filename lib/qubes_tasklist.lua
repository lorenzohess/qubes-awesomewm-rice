...
local my_theme = require("config").theme
...
local function tasklist_label(c, args, tb)
    ...
    -- Focused bg is its titlebar color
    local bg_focus = gcolor.ensure_pango_color(c.border_color)
    --local bg_focus = args.bg_focus or theme.tasklist_bg_focus or theme.bg_focus or bg_normal

    -- Unfocused bgs are bright black (widget bg)
    --local bg_normal = gcolor.ensure_pango_color("#5b6268")
    local bg_normal = gcolor.ensure_pango_color(c.border_color)
    --local bg_normal = args.bg_normal or theme.tasklist_bg_normal or theme.bg_normal or "#000000"

    -- Focused fg is the focused fg inverted to black or white
    local fg_focus = gcolor.ensure_pango_color(invertHex.invert(bg_focus, true))
    --local fg_focus = gcolor.ensure_pango_color(args.fg_focus or theme.tasklist_fg_focus or theme.fg_focus, fg_normal)

    -- Unfocused fgs is the focused bg inverted to black or white
    local fg_normal = gcolor.ensure_pango_color(invertHex.invert(bg_normal, true))
    ...
    local fg_minimize = gcolor.ensure_pango_color(args.fg_minimize or theme.tasklist_fg_minimize or theme.fg_minimize, fg_normal)
    ...
    --local shape_border_color = gcolor.ensure_pango_color(c.border_color)
    local shape_border_color = args.shape_border_color or theme.tasklist_shape_border_color
    local shape_border_width_focus = 1
    -- Draw border around button of focused window which is inverted color of
    -- the window's titlebar.
    local shape_border_color_focus = gcolor.ensure_pango_color(invertHex.invert(bg_focus))
    --local shape_border_color_focus = my_theme.tasklist_border_color_focus
    ...
    if focused then
        ...
        if shape_border_width_focus or args.shape_border_width_focus or theme.tasklist_shape_border_width_focus then
            shape_border_width = shape_border_width_focus or args.shape_border_width_focus or theme.tasklist_shape_border_width_focus
        end

        if shape_border_color_focus or args.shape_border_color_focus or theme.tasklist_shape_border_color_focus then
            shape_border_color = shape_border_color_focus or args.shape_border_color_focus or theme.tasklist_shape_border_color_focus
        end
