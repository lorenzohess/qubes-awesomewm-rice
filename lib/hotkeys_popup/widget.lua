...
function widget.new(args)
    ...
    function widget_instance:_load_widget_settings()
        ...
        self.width = args.width or dpi(500)
        ...
        -- The font for group names
        self.group_font = args.hotkeys_group_font or
            my_theme.hotkeys_group_font or "Monospace Bold 11"
        -- The keys to move up and down a page
        self.page_up_key = args.hotkeys_page_up_key or
            my_theme.hotkeys_page_up_key or "k"
        self.page_down_key = args.hotkeys_page_down_key or
            my_theme.hotkeys_page_down_key or "j"
    ...
    function widget_instance:_create_wibox(s, available_groups, show_awesome_keys)
        s = get_screen(s)
        local wa = s.workarea
        -- Set the wibox dimensions as 30% x 80% of screen workarea's width and height.
        --
        -- WARN: With 20% width and 80% height, the show_help() function raises the error:
        -- attempt to index a nil value (local 'previous_page_last_layout').
        --
        -- This variable is used only three times in this _create_wibox()
        -- function, at about line 430, in the "arrange columns into pages"
        -- section. I assume the error occurs because of setting the width
        -- to 20%, because the popup is drawn on my widescreen 21:9 monitor, but
        -- not the normal 16:9 one. To avoid the error, I set the width to 30%.
        local height = (wa.height * 0.80) - (self.border_width * 2)
        local width =  (wa.width  * 0.30) - (self.border_width * 2)
        ...
        for _, item in ipairs(column_layouts) do
            if item.max_width > available_width_px then
                previous_page_last_layout:add(
                    self:_group_label(self.page_down_key .. " - Next Page", self.label_bg)
                )
                table.insert(pages, columns)
                columns = wibox.layout.fixed.horizontal()
                available_width_px = width - item.max_width
                item.layout:insert(
                    1, self:_group_label(self.page_up_key .. " - Prev Page", self.label_bg)
                )
            else
    ...
    function widget_instance:show_help(c, s, show_args)
        ...
        help_wibox.keygrabber = awful.keygrabber.run(function(_, key, event)
            ...
            if key then
                if key == self.page_down_key then
                    help_wibox:page_next()
                elseif key == self.page_up_key then
